...
namespace com.fpnn.rtm
{
    public partial class RTMClient
    {
        ...
        private WebSocketClient rtmGate;
        ...
        public static RTMClient getInstance(string endpoint, long projectId, long uid, RTMQuestProcessor serverPushProcessor, bool autoRelogin = true)
        {
            ...
            lock (instanceLocker)
            {
                client = RTMControlCenter.Instance.FetchClient(projectId, uid);
                if (client != null)
                {
                ...
                else
                {
                    ...
                    RTMControlCenter.Instance.AddClient(projectId, uid, client);
                }
            }
            return client;
        }
        ...
        private WebSocketClient GetCoreClient()
        {
            if (status == ClientStatus.Connected)
                return rtmGate;
            else
        ...
        {
            if (originalEndpoint == null && rtmGate != null)
                originalEndpoint = rtmGate.Endpoint();
            else
                originalEndpoint += "/service/websocket";
                
            rtmGate = WebSocketClient.Create(originalEndpoint, true);

            if (errorRecorder != null)
                rtmGate.SetErrorRecorder(errorRecorder);

            rtmGate.SetQuestProcessor(processor);
            rtmGate.SetConnectionConnectedDelegate(() => {
                if (requireClose)
                {
                ...
                RTMControlCenter.Instance.RegisterSession(uid, this);
                Auth(true);
            });

            rtmGate.SetConnectionCloseDelegate(() => {
                ...
                if (autoReloginInfo != null)
                {
                    ...
                    autoReloginInfo.lastErrorCode = fpnn.ErrorCode.FPNN_EC_CORE_CONNECTION_CLOSED;
                }
                ...
                else
                {
                    if (startRelogin)
                        ...
                        processor.SessionClosed(fpnn.ErrorCode.FPNN_EC_OK);
                }
            });
        }
        ...
        private void StartNextRelogin()
        {
            ...
            RTMControlCenter.Instance.DelayRelogin(this, ClientEngine.GetCurrentMilliseconds() + interval);
        }
        ...
        //-------------[ Close interfaces ]--------------------------//
        internal void AsyncClose(bool disableRelogin)
        {
            bool isConnecting = false;
